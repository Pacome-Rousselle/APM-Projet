...
void my_masks_clone(masks_ptr *const restrict my_masks_ret_ptr,const masks_ptr orig_masks,wfc_blocks_ptr init ){
 ...
 int size = sizeof(orig_masks->block_masks) +sizeof(orig_masks->column_masks)+
                    sizeof(orig_masks->row_masks)+sizeof(orig_masks->safe_exit)+sizeof(masks); 
  masks_ptr ret = *my_masks_ret_ptr; 
  ret = malloc(size); 
  memcpy(ret,orig_masks, size); 
  ret->safe_exit = 0; 
  *my_masks_ret_ptr = ret; 
}
