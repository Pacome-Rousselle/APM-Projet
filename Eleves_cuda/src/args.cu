...
count_seeds(const seeds_list *restrict const seeds)
{
    ...
    for (uint64_t i = 0; i < seeds->count; i += 1) {
        ...
        case /*seed_item_single*/ 1: 
        total += 1; 
        break;
        case /*seed_item_tuple*/ 2 : {
...
seeds_list_pop(seeds_list *restrict *const list_ptr)
{
    ...
    // Handle tuples
    else if (list->items[0].type == /*seed_item_tuple*/ 2 ) {
    ...
    // Handle single items
    else if (list->items[0].type == /*seed_item_single*/ 1) {
    ...
    while ((opt = getopt(argc, argv, "hs:o:l:p:")) != -1) {
        ...
        case 's': {
            ...
            if ('\0' == *end) {
                ...
                seed_item new_item; 
                new_item.type = seed_item_tuple; 
                new_item.content.from = from; 
                new_item.content.to = to; 
                seeds = seeds_list_push_item(seeds, new_item); 
