...
bool
solve_openmp(wfc_blocks_ptr blocks, uint64_t seed, masks* my_masks)
{
    printf("inside solver omp \n"); 
    ...
    uint32_t row_idx  ;
    uint32_t block_idx;
    #pragma omp parallel for num_threads(blocks->grid_side*blocks->grid_side)
    forever {
        //printf("Itération %d\n",iteration);
        //while find_min_entropy in grid 
        loc.entropy = UINT8_MAX;
        test = blk_min_entropy(blocks,0,omp_get_thread_num());
        ...
        blocks->states[idx] = entropy_collapse_state(blocks->states[idx],
                                                    ...
                                                    seed,
                                                    iteration);
        col_idx   = omp_get_thread_num() * blocks->block_side + loc.location_in_blk.y; 
        row_idx   = 0 * blocks->block_side + loc.location_in_blk.x; 
        block_idx = 0 * blocks->grid_side  + omp_get_thread_num(); 
